export default function CompanyModificationSuccess({ user, close }) {
    const handleClose = () => {
      close("pending");
    };
    return (
      <div className="fixed inset-0 z-10 flex flex-col items-center backdrop-brightness-90 justify-center backdrop-blur-sm">
        <div className=" w-3/12 right-1/2 border-2 border-violet-900 top-1/4 max-h-80 bg-white justify-center flex flex-col items-center rounded-xl px-8 py-10 text-gray-800 shadow-lg">
          ...
          <p className="mt-4 text-center text-2xl font-bold">ENABLED</p>
          <p className="mt-2 text-center font-semibold text-lg">
            You enabled the user
          </p>
          <div className="mt-8 flex flex-col justify-center space-y-3 sm:flex-row sm:space-x-3 sm:space-y-0">
            <button
              onClick={handleClose}
              className="whitespace-nowrap rounded-md bg-green-600 px-4 py-3 font-medium text-white"
            >
              Continue
            </button>
          </div>
